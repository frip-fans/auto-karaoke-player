...
默认曲库为系统文档目录下的 `Karaoke/songs`；在“曲库位置与投屏”中可选择其他目录，选择后直接切换，并记住新位置。启动时只读取 `library.json` 索引，不遍历视频目录；直接放入曲库的新 MP4 可点击曲库栏的扫描按钮发现。桌面版优先监听 `127.0.0.1:8787`；如果开发服务等程序占用了该端口，会自动选择空闲端口，切换曲库时沿用本次启动的端口。曲库内容不受端口影响；队列、音量和界面偏好仍属于当前浏览器来源，使用备用端口时可能显示为独立记录。开发时从 PATH 或 `FFMPEG_PATH` / `FFPROBE_PATH` 寻找媒体工具，并只在扫描、导入或准备播放需要时启动它们。
